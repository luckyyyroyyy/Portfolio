...
def handle_chat(path):
    ...
    if request.method == 'GET':
        return jsonify({
            'status': 'online',
            'message': 'Lucky Roy Portfolio Chatbot API (Fast Gemini 2.5 Flash) is online.'
        })
    ...
    try:
        ...
        # Build contents array with recent history (last 6 messages max for low latency)
        contents = []
        ...
        # Fast Gemini payload with thinkingBudget=0 to eliminate reasoning delay
        payload = {
